...
Diagnostic script. Runs the tracker and depth model on a clip and reports what depth_model.py actually outputs on real footage, so that downstream detectors use it correctly as a relative cross-check.

Two questions to answer empirically:
  1. Direction. Is a near car's depth value larger or smaller than a far car's? Depth Anything V2 raw output can go either way depending on configuration, and our wrapper preserves the raw scale.
  2. Scale and range. Roughly what numbers the model produces, and how stable they are frame to frame for the same object.

Method. On each frame, take the vehicle with the lowest bbox bottom as the "near" candidate and the smallest-area vehicle as the "far" candidate. Print the median depth over a patch inside each bbox along with the global frame min and max, then summarize across the run.
...
def patch_median(depth: np.ndarray, x1, y1, x2, y2) -> float:
    """ Median depth over the lower-central third of the bbox. Targets the vehicle body and avoids sky or background visible through glass. """
    h, w = depth.shape[:2]
...
def main():
    ...
    while n < args.frames:
        ...
        note = ""
        if len(vehicles) >= 1:
            # NEAR: vehicle whose bbox bottom sits lowest in the frame
            ...
            # FAR: smallest-area vehicle as a proxy for farthest
    ...
    if near_vals and far_vals:
        ...
        print(f"median FAR-vehicle  depth : {fm:.3f}  "
              f"(n={len(far_vals)})")
        if nm > fm:
            print("NEAR > FAR: a larger depth value indicates a closer object")
        else:
            print("NEAR < FAR: a smaller depth value indicates a closer object")
